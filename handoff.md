...
**Mobile dashboards** (`Knowledge/Test/<file> - Mobile.md` in the iCloud vault) are standalone `dataviewjs` blocks **generated by the plugin** from templates in `main.ts` (`generateLibraryMobileDashboard`, `generateGenericMobileDashboard`, `generateHabitMobileDashboard`). They read the `_csv_helpers/<file>.csv` mirror that the plugin writes on `csv-refresh`.
