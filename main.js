...
var AddEntryModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    const submit = () => {
      ...
      if (!hasAny) {
        new import_obsidian.Notice("Fill in at least one field.");
        return;
      }
...
var NoteExpanderModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    header.createDiv({ cls: "csv-expander-title", text: (_b = this.row[(_a = this.headers.find((h) => ["title", "name", "Title", "Name"].includes(h))) != null ? _a : this.headers[0]]) != null ? _b : "\u2014" });
    ...
    this.headers.forEach((h) => {
      ...
      if (this.isSelectCol(h)) {
        ...
        val.addEventListener("click", () => {
          var _a2;
          val.empty();
          const input = val.createEl("input", { cls: "csv-inline-input", value: (_a2 = this.row[h]) != null ? _a2 : "", type: "text" });
          input.focus();
          input.select();
          const commit = () => {
            this.row[h] = input.value;
            val.empty();
    ...
    let isEditing = false;
    let currentText = (_c = this.row[this.notesCol]) != null ? _c : "";
    ...
    const renderMarkdown = () => {
      rendered.empty();
      if (currentText.trim()) {
        import_obsidian.MarkdownRenderer.render(this.app, currentText, rendered, this.filePath, this.renderComponent);
      } else {
    ...
    editBtn.addEventListener("click", () => {
      ...
      if (isEditing) {
        ...
        ta.value = currentText;
        ta.focus();
        editBtn.setText("\u{1F441} Preview");
      } else {
        editorWrap.style.display = "none";
...
var FileConfigModal = class extends import_obsidian.Modal {
  constructor(app, headers, filePath, current, onSave) {
    super(app);
    this.headers = headers;
    this.filePath = filePath;
  ...
  onOpen() {
    ...
    const makeDropdown = (label, currentVal, onChange) => {
      const row = form.createDiv({ cls: "csv-modal-row" });
      ...
      opts.forEach((o) => {
        ...
        if ((currentVal != null ? currentVal : none) === o)
          opt.selected = true;
      });
      sel.addEventListener("change", () => onChange(sel.value === none ? void 0 : sel.value));
    };
    makeDropdown("Category column (kanban grouping)", this.current.categoryColumn, (v) => {
      this.current.categoryColumn = v;
    });
    makeDropdown("Status column (row subgroups)", this.current.statusColumn, (v) => {
      this.current.statusColumn = v;
    });
    makeDropdown("Notes column", this.current.notesColumn, (v) => {
      this.current.notesColumn = v;
    });
    ...
    [["\u2014 use global default \u2014", ""], ["By Genre", "kanban-genre"], ["Table", "table"]].forEach(([label, val]) => {
      var _a;
      const opt = modeSel.createEl("option", { text: label, value: val });
      if (((_a = this.current.defaultMode) != null ? _a : "") === val)
        opt.selected = true;
    });
    modeSel.addEventListener("change", () => {
      this.current.defaultMode = modeSel.value ? modeSel.value : void 0;
    });
    const btnRow = contentEl.createDiv({ cls: "csv-modal-btns" });
...
var XLSXCardView = class extends import_obsidian.FileView {
  ...
  getDisplayText() {
    var _a, _b;
    return (_b = (_a = this.file) == null ? void 0 : _a.basename) != null ? _b : "Card View";
  }
  getIcon() {
    return "table";
  }
    ...
    try {
      if (this.isXlsx) {
        ...
        if (!raw.length) {
          ...
          this.headers = raw[0].map(String);
          this.rows = raw.slice(1).map((r) => {
            const row = {};
            this.headers.forEach((h, i) => {
              var _a2;
              row[h] = String((_a2 = r[i]) != null ? _a2 : "");
            });
            return row;
          });
        }
  ...
  getNotesCol() {
    if (this.fileCfg.notesColumn)
      return this.fileCfg.notesColumn;
  ...
  getStatusCol() {
    ...
    if (this.fileCfg.statusColumn) {
      return (_a = this.headers.find((h) => h.toLowerCase() === this.fileCfg.statusColumn.toLowerCase())) != null ? _a : null;
    }
    return this.resolveCol([
      "Status",
      "status",
  ...
  getCategoryCol() {
    ...
    if (this.fileCfg.categoryColumn) {
      return (_a = this.headers.find((h) => h.toLowerCase() === this.fileCfg.categoryColumn.toLowerCase())) != null ? _a : null;
    }
    return this.resolveCol([
      "Category",
      "category",
  ...
  titleKey() {
    var _a;
    return (_a = this.resolveCol(["Title", "title", "Name", "name"])) != null ? _a : void 0;
  }
  authorKey() {
    var _a;
    return (_a = this.resolveCol([
      "Author",
      "author",
      "Authors",
      "authors",
  ...
  getTitle(row) {
    var _a;
    const k = this.titleKey();
    return (_a = k ? row[k] : row[this.headers[0]]) != null ? _a : "\u2014";
  }
  getSubtitle(row) {
    var _a;
    const k = this.authorKey();
    return k ? (_a = row[k]) != null ? _a : "" : "";
  }
  getColumnValues(h) {
    return Array.from(new Set(this.rows.map((r) => {
      var _a;
      return (_a = r[h]) != null ? _a : "";
    }).filter(Boolean))).sort();
  }
  ...
  async openOrCreateNotes(row) {
    ...
    if (!file) {
      ...
      const content = fm + (notesCol && ((_a = row[notesCol]) == null ? void 0 : _a.trim()) ? row[notesCol] : "");
  ...
  renderSelectField(container, row, h) {
    ...
    chip.addEventListener("click", (e) => {
      ...
      showSelectPicker(chip, val, this.getColumnValues(h), (newVal) => {
        ...
        if (h.toLowerCase() === "status" && newVal)
          chip.addClass(`status-chip-${newVal.toLowerCase().replace(/\s+/g, "-")}`);
        this.scheduleSave();
      }, this.contentEl);
    });
    return chip;
  ...
  renderView() {
    ...
    if (!this.headers.length) {
      content.createEl("p", { text: "Empty or unreadable file.", cls: "csv-empty-state" });
      return;
    }
    if (this.mode === "kanban-genre")
  ...
  renderToolbar(root) {
    ...
    [{ id: "kanban-genre", label: "By Genre" }, { id: "table", label: "Table" }].forEach(({ id, label }) => {
    ...
    cfgBtn.addEventListener("click", () => {
      var _a2, _b2;
      new FileConfigModal(this.app, this.headers, (_b2 = (_a2 = this.file) == null ? void 0 : _a2.path) != null ? _b2 : "", this.fileCfg, (cfg) => {
  ...
  renderKanbanGenre(container) {
    ...
    if (!cc) {
      container.createEl("p", { text: `No "${this.settings.categoryColumn}" column found.`, cls: "csv-empty-state" });
      return;
    }
    const genreSet = /* @__PURE__ */ new Set();
    this.rows.forEach((r) => {
      var _a;
      return ((_a = r[cc]) != null ? _a : "").split(",").map((s) => s.trim()).filter(Boolean).forEach((c) => genreSet.add(c));
    });
    const genres = Array.from(genreSet).sort();
    if (!genres.length) {
      container.createEl("p", { text: "No genre values found.", cls: "csv-empty-state" });
      return;
    }
    const statusOrder = ["In progress", "Finished", "Not started"];
    const statuses = sc ? Array.from(/* @__PURE__ */ new Set([...statusOrder, ...this.rows.map((r) => {
      var _a;
      return (_a = r[sc]) != null ? _a : "";
    }).filter(Boolean)])).filter((s) => this.rows.some((r) => {
      var _a;
      return ((_a = r[sc]) != null ? _a : "") == s;
    })) : [];
    const board = container.createDiv({ cls: "csv-kanban-board" });
    genres.forEach((genre) => {
      const genreRows = this.rows.filter((r) => {
        var _a;
        return ((_a = r[cc]) != null ? _a : "").split(",").map((s) => s.trim()).includes(genre);
      });
      if (!genreRows.length)
        return;
      ...
      if (sc && statuses.length) {
        statuses.forEach((status) => {
          const statusRows = genreRows.filter((r) => {
            var _a;
            return ((_a = r[sc]) != null ? _a : "") === status;
          });
  ...
  // ── Kanban card ────────────────────────────────────────────────────────────
  renderKanbanCard(container, row, statuses, sc) {
    var _a;
    ...
    if (sub)
      card.createDiv({ cls: "csv-kanban-card-sub", text: sub });
    const tk = this.titleKey(), ak = this.authorKey(), ccol = this.getCategoryCol();
    const skipInCard = new Set([sc, tk, ak, ccol].filter(Boolean));
    const metaEl = card.createDiv({ cls: "csv-kanban-card-meta" });
    this.headers.forEach((h) => {
      if (skipInCard.has(h) || this.isNotesCol(h) || !row[h])
        return;
    ...
    const notesCol = this.getNotesCol();
    const hasInlineNotes = !!(notesCol && ((_a = row[notesCol]) == null ? void 0 : _a.trim()));
    ...
    const openInlineEditor = () => {
      ...
      ta.value = (_c = notesCol ? row[notesCol] : "") != null ? _c : "";
    ...
    if (notesCol) {
      ...
      btnRow.createEl("button", { cls: "csv-kanban-notes-btn", text: "\u2922 Expand" }).addEventListener("click", (e) => {
        e.stopPropagation();
        this.openNoteExpander(row, notesCol);
      });
    }
    btnRow.createEl("button", { cls: `csv-kanban-notes-btn ${hasFile ? "" : "csv-kanban-create-btn"}`, text: hasFile ? "\u{1F4C4} Open notes file" : "\u271A Notes file" }).addEventListener("click", (e) => {
      e.stopPropagation();
      this.openOrCreateNotes(row);
    });
    ...
    card.addEventListener("contextmenu", (e) => {
      ...
      if (sc) {
        ...
        statuses.forEach((s) => {
          if (s === row[sc])
            return;
          menu.addItem((i) => i.setTitle(`Mark as: ${s}`).onClick(() => {
            row[sc] = s;
            this.scheduleSave();
            this.renderView();
          }));
    ...
    this.rows.forEach((row, idx) => {
      ...
      this.headers.forEach((h) => {
        ...
        if (this.isNotesCol(h)) {
          ...
          const preview = ((_a = row[h]) != null ? _a : "").replace(/#{1,6}\s/g, "").replace(/[*_>`]/g, "").split("\n").filter((l) => l.trim()).slice(0, 2).join(" \xB7 ");
          ...
          const openExpander = (e) => {
            e.stopPropagation();
            this.openNoteExpander(row, h);
          };
          ...
          td.setText((_b = row[h]) != null ? _b : "");
          this.makeEditable(td, row, h);
        }
      });
      const at = tr.createEl("td", { cls: "csv-table-action" });
      const hasFile = this.notesFileExists(row);
      at.createEl("button", { cls: `csv-table-notes-btn ${hasFile ? "exists" : ""}`, text: hasFile ? "\u{1F4C4}" : "\u271A", title: hasFile ? "Open notes" : "Create notes" }).addEventListener("click", () => this.openOrCreateNotes(row));
      at.createEl("button", { cls: "csv-table-del-btn", text: "\u2715" }).addEventListener("click", () => {
  ...
  makeEditable(el, row, h) {
    el.addEventListener("click", () => {
      var _a;
      el.empty();
      const input = el.createEl("input", { cls: "csv-inline-input", value: (_a = row[h]) != null ? _a : "", type: "text" });
      input.focus();
      input.select();
